...
You can create associations between a customer and one or more carts. After cart associations exist for a customer, the customer can access those carts across any device. You can delete associations as required.

There are two ways to access the customer: with a `customer` token and without one.
...
Customers create carts and can use any of the carts that they created to check out an order.

:::note
You can create a cart id, name, and description for the cart. The cart requires a name. Ensure that the string length is greater than or equal to one. Use any symbol in the name and description. For cart id, ensure that you follow the guidelines for safe characters. For more information about cart id naming requirements, see [Safe Characters](/docs/api-overview/safe-characters).
:::
...
* `400` - `The type does not exist or is not listed as customer` - Please ensure that the type is `customer` and is present.

* `403` - `Cannot associate more than one customer`.

* `403` - `Customer does not have the required permissions to fulfill this request`.

* `403` - `Invalid json payload` - Please check JSON input: The request body must be an array `[]`. If the request body is an object, the error is thrown.
