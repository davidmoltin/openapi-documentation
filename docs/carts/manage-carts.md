...
An explicit action is when a user calls the cart creation endpoint. For more information, see [Create a Custom Cart](/docs/carts/create-custom-cart-items).
...
When a shopper signs in to their account, a cart association is created between the shopper’s account and the cart. For this action, the front-end calls the cart association API. For more information, see [Customer Cart Associations](/docs/carts/customer-cart-associations/customer-cart-associations-overview).
