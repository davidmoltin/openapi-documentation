...
After a [cart](/docs/carts/cart-management/cart-management-overview) is ready to checkout, you can easily convert your cart to an [order](/docs/orders/orders-api/orders-api-overview). The cart remains and can be modified and checked out again if required.
...
- To ensure that a free gift is automatically applied to an order, the promotion must be set to automatic. The checkout process will not be successful if free gift items are out of stock. See [Errors](#errors) section.
...
:::caution
There are a number of actions that happen to your [inventory](/docs/pxm/inventories/inventory-api-overview) when checking out and paying for an order. For more information please be sure to evaluate our [detailed article](/docs/pxm/inventories/inventory) explaining the processes.
:::
...
| `shipping_address.county`       | Optional | `string` | Specifies the county of the shipping address. |
...
- [Paying for an order](/docs/payment/paying-for-an-order/overview)
