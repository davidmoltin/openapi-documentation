...
- An event key is the name of the Commerce event that triggers the event mapping configuration. For example, `order.paid` or `order.created`. See [Observable events](/docs/integrations/observable-events) for a list of Commerce events.
...
* string
* The payload delivered to your webhook from Commerce contains the information about the resources affected by the fired event. See [Integration Payload](/docs/integrations/integration-payload). The payload must include: 
