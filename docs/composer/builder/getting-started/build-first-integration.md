...
Most integrations are either scheduled events or webhooks triggered by an Elastic Path [observable event](/docs/integrations/observable-events). For this tutorial, we are using the **Webhook Trigger**.
...
1. Click the step to display the drawer details. By default the following settings are set. 
	...
	1. In **Inputs**, **Response Type** is set to **Asychronous**.
...
1. Click **Field**. The **Field** details are dislayed.
